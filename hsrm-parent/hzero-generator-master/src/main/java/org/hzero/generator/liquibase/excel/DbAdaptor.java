...
import org.hzero.generator.liquibase.excel.TableData.TableCellValue;
...
import org.hzero.generator.liquibase.addition.AdditionDataSource;
import org.hzero.generator.liquibase.exception.LiquibaseException;
import org.hzero.generator.liquibase.helper.LiquibaseHelper;
import org.hzero.generator.liquibase.utils.CellDataConverter;
...
public class DbAdaptor {
    ...
    Map<String, String> tableDeleteSqlMap = new HashMap<>();
    Map<String, String> tableDeleteTlSqlMap = new HashMap<>();
    ...
    protected int doDelete(TableData.TableRow tableRow) throws SQLException {
        ...
        List<TableCellValue> uniques = new ArrayList<>();
        for (TableCellValue tableCellValue : tableRow.getTableCellValues()) {
            if (tableCellValue.getColumn().isGen() || tableCellValue.isValuePresent()) {
                genTableCellValue = tableCellValue;
                break;
            }
            if (tableCellValue.getColumn().isUnique()) {
        ...
        //删除sql
        String sql = prepareDeleteSql(tableRow, uniques);
        Connection connection = this.connection;
        if (connectionMap.containsKey(tableRow.getTable().getName())) {
            connection = connectionMap.get(tableRow.getTable().getName());
        }
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            //删除记录并设置状态不存在
            tableRow.setExistsFlag(false);
        } catch (SQLException e) {
            logger.error("[{}]delete error, row:{} ,sql:{}",
                    tableRow.getTable().getName(), tableRow, sql);
            throw e;
        }

        logger.info("delete row:{}", tableRow);
        tableRow.getTable().setDelete(tableRow.getTable().getDelete() + 1);
        if (genTableCellValue == null || tableRow.getTable().getLangs().isEmpty()) {
            return deleteCount;
        }
        //删除多语言表
        sql = prepareDeleteTlSql(tableRow.getTable().getName(), genTableCellValue);
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            int index = 1;
            setParam(ps, genTableCellValue, index++);
            deleteCount += ps.executeUpdate();
        } catch (SQLException e) {
            logger.error("[{}]error delete tl ,row:{},sql:{}",
                    tableRow.getTable().getName(), tableRow, sql);
            throw e;
        }
        logger.info("delete tl for row: {}", tableRow);
        return deleteCount;
    }
            ...
            for (TableData.TableCellValue tableCellValue : tableRow.getTableCellValues()) {
                if (tableCellValue.getColumn().isGen()) {
                    ...
                    if (isGeneratedColumnInserted) {
                        pkColumnName = tableCellValue.getColumn().getName();
                        expectedPk = Long.valueOf(genTableCellValue.getValue());
                        ps.setLong(count++, expectedPk);
                        //记录当前插入数据手动设置id的最大值
                        table.setMaxId(expectedPk > maxId ? expectedPk : maxId);
                    } else if (sequencePk()) {
            ...
            if (!sequencePk() && genTableCellValue != null) {
                try (ResultSet rs = ps.getGeneratedKeys()) {
                    if (rs != null && rs.next()) {
                        try {
                            genVal = rs.getLong(1);
                            if (expectedPk != null && genVal != expectedPk && StringUtils.isNotEmpty(pkColumnName)) {
                                PreparedStatement updatePk = connection.prepareStatement(String.format("update %s set %s = ? where %s = %d", tableName, pkColumnName, pkColumnName, genVal));
                                updatePk.setLong(1, expectedPk);
                                updatePk.executeUpdate();
                                genVal = expectedPk;
                            }
                        } catch (SQLException e) {
                            logger.error("Error get pk value, If you want to see the details, please set the log [io.choerodon.liquibase.excel] level to debug : {}", tableRow);
    ...
    private List<TableCellValue> getUnpresentFormulaTds(TableData.TableRow tableRow) {
        List<TableCellValue> formulaTableCellValues = new ArrayList<>();
        for (TableData.TableCellValue tableCellValue : tableRow.getTableCellValues()) {
            if (tableCellValue.isFormula() && !tableCellValue.isValuePresent()) {
    ...
    public int weakInsert(List<TableData> tables) throws SQLException {
        ...
        do {
            ...
            for (TableData.TableRow tableRow : tempList) {
                ...
                List<TableCellValue> formulaTableCellValues =
    ...
    protected int doInsertTl(TableData.TableRow tableRow, String lang) throws SQLException {
        ...
        List<TableCellValue> values = new LinkedList<>();
    ...
    protected String prepareDeleteSql(TableData.TableRow tableRow,
                                      List<TableCellValue> uniques) {
        String sql = tableDeleteSqlMap.get(tableRow.getTable().getName());
        if (sql == null) {
            StringBuilder sb = new StringBuilder();
            sb.append(SQL_DELETE).append(tableRow.getTable().getName()).append(SQL_WHERE);
            for (TableCellValue tableCellValue : uniques) {
                sb.append(tableCellValue.getColumn().getName());
                sb.append("=? AND ");
            }
            sb.delete(sb.length() - 4, sb.length());
            sql = sb.toString();
            tableDeleteSqlMap.put(tableRow.getTable().getName(), sql);
        }
        return sql;
    }

    protected String prepareDeleteTlSql(String tableName, TableCellValue genTableCellValue) {
        String sql = tableDeleteTlSqlMap.get(tableName);
        if (sql == null) {
            StringBuilder sb = new StringBuilder();
            sql = sb.append(SQL_DELETE).append(tlTableName(tableName)).append(SQL_WHERE)
                    .append(genTableCellValue.getColumn().getName()).append("=?").toString();
            tableDeleteTlSqlMap.put(tableName, sql);
        }
        return sql;
    }
