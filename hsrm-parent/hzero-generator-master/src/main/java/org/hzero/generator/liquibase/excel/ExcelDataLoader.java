...
public class ExcelDataLoader {
    ...
    TableData.TableCellValue findCell(String cellNum, TableData.TableCellValue tableCellValue) {
        ...
        for (TableData tableData : tables) {
            ...
            if (sl < row && sl + tableData.getTableRows().size() >= row) {
                ...
                if (tableRow.isDeleteFlag() && !tableCellValue.getTableRow().canDelete()) {
                    throw new LiquibaseException("invalid reference:" + sheetName + cellNum + " ,deleted tableRow:" + tableRow);
                }
     ...
     */
    public void execute(InputStream inputStream, AdditionDataSource ad) throws LiquibaseException {
        try {
